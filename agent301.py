...
def print_welcome_message():
    print(Fore.GREEN + Style.BRIGHT + "Agent 301 BOT by. EDD")
def user(auth_token):
    url = "https://s2.agent301.org/getMe"
    ...
    return balance, tasks

def clear_task(auth_token, task_type):
    url = "https://s2.agent301.org/completeTask"
    headers = HEADERS.copy()
    headers["authorization"] = auth_token
    body = {"type": task_type}
                ...
                for task in tasks:
                    ...
                    if task_type == "video":
                        ...
                        if not task.get('is_claimed'):
                            print(f"[ {Fore.LIGHTYELLOW_EX}INFO{Style.RESET_ALL} ] Attempting to complete task: {Fore.LIGHTMAGENTA_EX}{task_title}{Style.RESET_ALL}")
                            futures.append(executor.submit(process_task, auth_token, task_type, task_title, 1))
